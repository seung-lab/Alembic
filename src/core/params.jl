...
function load_params(fn)
	...
	global PARAMS = Dict(
		 :dirs => Dict(
		 	...
		 	:alembic.mesh => joinpath(root, dirs["dst_image"], dirs["mesh"]),
		 	:alembic.match => joinpath(root, dirs["dst_image"], dirs["match"]),
		 	:alembic.meshset => joinpath(root, dirs["dst_image"], dirs["meshset"]),
		 	:cache => dirs["cache"]
			),
	     :mesh => Dict(
